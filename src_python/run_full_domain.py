...
import sys
sys.path.append('./src')
from ratchet_reset_full_domain import Particle
...
ax.hlines(0, 0, total_t, color='black', linestyle='--', alpha=0.2, linewidth=5)
ax.hlines(L, 0, total_t, color='black', linestyle='--', alpha=0.2, linewidth=5)
ax.hlines(2*L, 0, total_t, color='black', linestyle='--', alpha=0.2, linewidth=5)
...
ax.set_yticks([0,1,2])
ax.set_yticks(ax.get_yticks(), [0, r'$L$', r'$2L$'])
ax.set_ylim(-0.2, 2.1)
...
# ax1 = fig.add_axes([1.1, 1.1, 0.2, 1.0])

# ax1.plot([0, 1], [0, 1], color='tab:blue', label='D')
...
if not os.path.exists(folder):
    os.makedirs(folder)
filename = 'L{}_a{}_h{}_D{}_r{}_seed{}_unwrapped'.format(L, a, h, D, r, seed)
